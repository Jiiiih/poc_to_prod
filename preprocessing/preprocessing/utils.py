...
class BaseTextCategorizationDataset:
    ...
    def _get_num_train_samples(self):
        ...
        """
        #debug
        print("num train samples", self._get_num_samples()*self.train_ratio)
    ...
    def _get_num_train_batches(self):
        ...
        """
        #debug
        print("num train batches", self._get_num_train_samples()/self.batch_size)
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        assert dataset.columns.tolist()==columns_expected
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                """
                counts = df['tag_id'].value_counts()
                cleaned = df[df['tag_id'].isin(counts[counts >= x].index)]
                return cleaned
